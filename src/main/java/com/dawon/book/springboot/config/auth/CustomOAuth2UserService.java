...
import com.dawon.book.springboot.config.auth.dto.OAuthAttributes;
import com.dawon.book.springboot.config.auth.dto.SessionUser;
import com.dawon.book.springboot.domain.user.User;
import com.dawon.book.springboot.domain.user.UserRepository;
...
public class CustomOAuth2UserService  implements OAuth2UserService<OAuth2UserRequest, OAuth2User> {
    ...
    public OAuth2User loadUser(OAuth2UserRequest userRequest) throws OAuth2AuthenticationException {
        ...
        // registrationId : 현재 로그인 진행중인 서비스를 구분하는 코드. (구글,네이버, 다음...)
        String registrationId = userRequest.getClientRegistration().getRegistrationId();
        // userNameAttributeName : OAuth2 로그인 진행 시 키가 되는 필드값을 의미. pk와 같은 의미.
        String userNameAttributeName = userRequest.getClientRegistration().getProviderDetails().getUserInfoEndpoint().getUserNameAttributeName();

        // OAuthAttributes : OAuth2UserService를 통해 가져온 OAuthUser의 Attibute를 담은 클래스
